...
// vectorizerModule returns the module configured for the running Weaviate.
// Defaults to "none" (what brew install gives you without extra setup).
// Set WEAVIATE_MODULE=text2vec-contextionary to enable semantic tests.
...
func TestMain(m *testing.M) {
	...
	_ = rawClient.Schema().ClassDeleter().WithClassName(testClass).Do(context.Background())
	os.Exit(code)
}
...
func setupSchema() {
	...
	// Remove leftover from a previous interrupted run.
...
func seedData() {
	...
	// Let the index settle before running queries.
	time.Sleep(300 * time.Millisecond)
}

// newHandler returns a WeaviateHandler configured against the test instance.
...
// requiresVectorizer skips the test when no module is available.
...
// ---------------------------------------------------------------------------
// Unit tests — no Weaviate connection needed
// ---------------------------------------------------------------------------
...
func TestCapabilities(t *testing.T) {
	...
	for _, want := range []string{"search", "hybrid_search"} {
		if !actions[want] {
...
func TestConfigure_missingCollection(t *testing.T) {
	err := (&WeaviateHandler{}).Configure(`{"host":"localhost:8080"}`)
	if err == nil {
...
// ---------------------------------------------------------------------------
// Integration tests — require Weaviate running
// ---------------------------------------------------------------------------
...
func TestHybridSearch_keywordOnly(t *testing.T) {
	...
	resp := h.Execute(plugin.Request{
		...
		Args: map[string]string{
			"query": "python",
			"alpha": "0", // pure keyword (BM25) — works without any vectorizer
		},
	})
...
func TestHybridSearch_limitOverride(t *testing.T) {
	...
	// Drill into GraphQL response: data.Get.<ClassName>[...]
...
func TestHybridSearch_fieldsOverride(t *testing.T) {
	...
	resp := h.Execute(plugin.Request{
		...
		Args: map[string]string{
			"query":  "go",
			"alpha":  "0",
			"fields": "title", // only title, no body
		},
	})
	...
	// body field should not appear when only title is requested
